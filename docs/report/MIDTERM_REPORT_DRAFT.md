...
- **Tính toán nặng phía máy chủ:** Bản chất duyệt không gian tìm kiếm tổ hợp dạng mạng lưới (*lattice*) đòi hỏi nhiều lượt duyệt và so khớp tập con, dễ dẫn đến nghẽn tài nguyên CPU và bộ nhớ khi ngưỡng hỗ trợ tối thiểu bị hạ thấp.
...
- Tầng phụ trợ (Back-end) phát triển bằng PHP thuần hướng đối tượng với kiến trúc phân lớp nhẹ, triển khai thuật toán Apriori với cơ chế đo lường chi tiết động thái cắt tỉa qua từng bậc độ dài $k$.
- Tầng lưu trữ (Database) sử dụng MySQL với lược đồ quan hệ chuẩn hóa cao độ (`datasets`, `transactions`, `transaction_items`, `experiment_runs`, `experiment_run_levels`), tối ưu hóa chỉ mục cho các truy vấn phân tích.
...
### 3.1 Mục Tiêu Dự Án
- **Mục tiêu 1:** Xây dựng ứng dụng web quản lý tập dữ liệu, cấu hình khai phá và hiển thị bảng điều khiển tương tác bằng PHP, MySQL, Bootstrap và Apache ECharts.
...
- **RQ3 (So sánh Hiệu năng Trực quan hóa Front-End):**  
  *Ba thư viện trực quan hóa web D3.js (triển khai SVG), Chart.js (triển khai Canvas) và Apache ECharts (triển khai Canvas) so sánh như thế nào về độ trễ khởi tạo biểu đồ ban đầu ($\text{initial render}$) và độ trễ cập nhật dữ liệu tại chỗ ($\text{in-place data update}$) dưới các khối lượng dữ liệu điểm phân tán tương đương?*
...
Trong tài liệu khai phá dữ liệu, thuật toán FP-Growth (*Frequent Pattern Growth*) của Han và cộng sự (2000) được giới thiệu phổ biến như một giải pháp thay thế tránh việc sinh ứng viên tường minh:
- **Nguyên lý:** FP-Growth nén dữ liệu giao dịch vào một cấu trúc cây tiền tố liên kết bộ nhớ (*FP-tree*), sau đó sử dụng chiến lược chia để trị (*divide-and-conquer*) để khai phá các mẫu phổ biến từ các cây điều kiện (*conditional pattern bases*).
- **Đánh đổi lý thuyết:** Trong khi FP-Growth tránh việc sinh ứng viên tổ hợp, Apriori lại thể hiện rõ ràng và minh bạch cấu trúc phân tầng $k$-itemset, cho phép quan sát trực tiếp động thái cắt tỉa tổ hợp qua từng bước. Trong khuôn khổ đề tài này, Apriori được lựa chọn hiện thực hóa chính thức nhằm phục vụ mục tiêu phân tích chẩn đoán cơ chế cắt tỉa; FP-Growth được trình bày như nền tảng đối sánh lý thuyết; hiệu năng so sánh phụ thuộc vào tập dữ liệu và cách hiện thực hóa cụ thể.
...
- **D3.js (Data-Driven Documents):** Trong thực nghiệm RQ3, D3.js 7.9.0 được triển khai sử dụng công nghệ SVG DOM, nơi mỗi điểm dữ liệu được ánh xạ thành một phần tử `<circle>`.
- **Chart.js & Apache ECharts:** Trong thực nghiệm RQ3, Chart.js 4.4.8 và Apache ECharts 5.6.0 đều được cấu hình sử dụng công nghệ HTML5 Canvas rasterization.
...
┌──────────────────────────────▼──────────────────────────────┐
│                    TẦNG XỬ LÝ BACK-END                      │
│      PHP 8.2+ (Kiến trúc phân lớp nhẹ, autoloader riêng)    │
...
- `App\Mining\AprioriEngine`: Động cơ lõi điều phối thuật toán Apriori trong bộ nhớ.
...
- `App\Mining\AssociationRuleGenerator`: Dịch vụ sinh luật kết hợp và tính toán support, confidence, lift.
...
4. `AprioriEngine` thực thi thuật toán Apriori trực tiếp trên mảng giao dịch trong bộ nhớ và ghi nhận thời gian chạy `runtime_ms`.
...
7. `MiningResponseAssembler` lắp ráp dữ liệu trực quan hóa Top-N và phát phản hồi JSON (RFC 8259) về client để cập nhật giao diện.

### 6.3 Lược Đồ Cơ Sở Dữ Liệu và Chính Sách Lưu Trữ (ADR-002)
Lược đồ cơ sở dữ liệu MySQL 8.4 InnoDB bao gồm 5 bảng chuẩn:
...
- `POST /api/mining.php`: Tiếp nhận yêu cầu khai phá dữ liệu (Content-Type: application/json). Endpoint này không sử dụng tham số truy vấn `action`.
...
- Thời gian thực thi `runtime_ms` đo lường thuần túy thời gian thuật toán Apriori chạy trên tập giao dịch đã nạp sẵn trong bộ nhớ RAM, tách biệt hoàn toàn khỏi thời gian nạp cơ sở dữ liệu, thời gian sinh luật kết hợp và thời gian dựng giao diện.
...
- Ba thư viện đối chứng: **D3.js v7.9.0** (triển khai SVG), **Chart.js v4.4.8** (triển khai Canvas), **Apache ECharts v5.6.0** (triển khai Canvas).
...
- **Bậc $k \ge 3$ (`join_prune`):** Cắt tỉa tập con thực tế bắt đầu loại bỏ ứng viên từ bậc $k = 3$ và phát huy hiệu quả ở các bậc cao:
...
2. **Vùng Dữ Liệu Dày Đặc ($N \ge 5,000$):**  
   Khi khối lượng điểm tăng lên $10,000$, Chart.js ghi nhận độ trễ trung vị thấp nhất ($70.550\text{ ms}$ khởi tạo / $60.950\text{ ms}$ cập nhật), D3.js ghi nhận mức trung gian ($138.600\text{ ms}$ khởi tạo / $117.700\text{ ms}$ cập nhật), và Apache ECharts đạt $222.600\text{ ms}$ khởi tạo / $195.800\text{ ms}$ cập nhật dưới chế độ vẽ tiêu chuẩn. Xu hướng thay đổi này nhất quán với các khác biệt về kiến trúc dựng hình và chi phí xử lý khung làm việc của từng thư viện; thực nghiệm đối chứng này không phân lập riêng lẻ các cơ chế nguyên nhân nội bộ.

3. **So Sánh Cập Nhật Dữ Liệu Tại Chỗ vs. Khởi Tạo Lần Đầu:**  
   Tại các quy mô lớn ($N = 5,000$ và $N = 10,000$), độ trễ cập nhật dữ liệu tại chỗ thấp hơn độ trễ khởi tạo ban đầu trên cả ba thư viện (ví dụ D3.js từ $138.600\text{ ms}$ giảm xuống $117.700\text{ ms}$; Chart.js từ $70.550\text{ ms}$ giảm xuống $60.950\text{ ms}$).
...
- **Vai trò bảo vệ của Cắt tỉa Apriori:** Tỷ lệ cắt tỉa tăng dần lên gần $30\%$ cho thấy tính chất Apriori đã ngăn chặn hàng trăm ứng viên không phổ biến tham gia vào bước đánh giá độ hỗ trợ trong bộ nhớ. Điều này khẳng định giá trị của việc hiện thực hóa mô-đun cắt tỉa tối ưu trong các dịch vụ web backend.
...
1. **Phạm Vi Tập Dữ Liệu Đơn Lẻ:** Toàn bộ quan sát hình thức được thực hiện trên tập dữ liệu chuẩn UCI Mushroom (`agaricus-lepiota.data`, $N = 8,124$). Các đặc tính phân phối của dữ liệu giao dịch thương mại thưa (*sparse market basket data*) có thể mang lại động thái cắt tỉa khác biệt.
...
1. **Ràng Buộc Kiến Trúc Dựng Hình:** D3.js được triển khai với SVG DOM trong khi Chart.js và ECharts sử dụng Canvas. Kiến trúc dựng hình là một phần cấu thành của đối tượng nghiên cứu, không phải một biến số thuật toán cô lập hoàn toàn.
...
- Xây dựng thành công hệ thống ứng dụng web bảng điều khiển tương tác, tích hợp giữa tầng xử lý nghiệp vụ PHP phân lớp nhẹ, cơ sở dữ liệu MySQL và giao diện người dùng ECharts/Bootstrap đáp ứng.
