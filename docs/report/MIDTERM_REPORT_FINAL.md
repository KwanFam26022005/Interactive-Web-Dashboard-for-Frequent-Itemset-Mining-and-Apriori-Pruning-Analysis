...
Báo cáo này trình bày quá trình thiết kế, triển khai và đánh giá thực nghiệm một hệ thống ứng dụng web phục vụ bài toán khai phá tập mục phổ biến (*Frequent Itemset Mining — FIM*) [1] và khai phá luật kết hợp (*Association Rule Mining*) [1], [2]. Đề tài hướng tới việc kết hợp thuật toán khai phá dữ liệu với giao diện bảng điều khiển tương tác trên nền tảng web.
...
Trong phân tích dữ liệu, việc phát hiện các mẫu tri thức và mối tương quan trong các cơ sở dữ liệu giao dịch lớn đóng vai trò quan trọng trong nhiều lĩnh vực ứng dụng, từ phân tích giỏ hàng thương mại (*market basket analysis*), hệ thống gợi ý, đến phân tích dữ liệu sinh học. Khai phá tập mục phổ biến (*Frequent Itemset Mining — FIM*) và khai phá luật kết hợp (*Association Rule Mining*), khởi xướng bởi Agrawal, Imieliński, Swami (1993) [1] và Agrawal, Srikant (1994) [2], là một trong những nền tảng lý thuyết cơ bản của lĩnh vực khai phá dữ liệu.

Mặc dù nền tảng lý thuyết của thuật toán Apriori [2] đã được nghiên cứu sâu rộng, việc ứng dụng thuật toán này trong môi trường web đối mặt với những thách thức kỹ thuật:
- **Tải tính toán phía máy chủ:** Bản chất duyệt không gian tìm kiếm tổ hợp dạng mạng lưới (*lattice*) [4] đòi hỏi nhiều lượt kiểm tra và so khớp tập con, làm tăng thời gian xử lý khi ngưỡng hỗ trợ tối thiểu bị hạ thấp.
- **Tương tác người dùng:** Đa số các công cụ khai phá truyền thống hoạt động dưới dạng dòng lệnh (*Command-Line Interface — CLI*) hoặc các thư viện độc lập, tạo khoảng cách đối với người dùng cần tương tác trực tiếp với tham số và quan sát kết quả trực quan.
- **Hiển thị phía máy khách (Client-Side Rendering):** Các tập mẫu phổ biến và luật kết hợp khi trích xuất có thể tạo ra khối lượng lớn điểm dữ liệu. Việc trực quan hóa các mối quan hệ đa chiều (Support vs. Confidence vs. Lift) trên trình duyệt web đòi hỏi cấu hình đồ họa phù hợp nhằm duy trì khả năng tương tác.

Đề tài này xây dựng một hệ thống ứng dụng web tích hợp:
- Tầng phụ trợ (Back-end) phát triển bằng PHP thuần hướng đối tượng [13] với kiến trúc phân lớp nhẹ, triển khai thuật toán Apriori [2] kèm cơ chế thu thập số liệu cắt tỉa chi tiết qua từng bậc độ dài $k$.
- Tầng lưu trữ (Database) sử dụng MySQL [14] với lược đồ quan hệ chuẩn hóa (`datasets`, `transactions`, `transaction_items`, `experiment_runs`, `experiment_run_levels`).
- Tầng giao diện (Front-end) là bảng điều khiển web tương tác ứng dụng AJAX, Bootstrap 5 [11], jQuery [12] và Apache ECharts [6], [10].
- Khung nghiên cứu thực nghiệm (Empirical Benchmark Pipeline) tự động hóa, đóng băng dữ liệu và kiểm chứng tính tái lập.
...
1. **Phương diện Kỹ thuật Web & Hệ thống:**  
   Thiết kế kiến trúc web có khả năng tiếp nhận các tệp dữ liệu giao dịch, tiền xử lý và lưu trữ, đồng thời cung cấp giao diện lập trình ứng dụng (HTTP JSON API) phi trạng thái để tiếp nhận yêu cầu khai phá Apriori từ giao diện người dùng. Trình duyệt gửi yêu cầu AJAX bất đồng bộ để tránh tải lại toàn trang, trong khi máy chủ xử lý đồng bộ và trả kết quả JSON với dữ liệu rút gọn để bảng điều khiển cập nhật giao diện tương tác.

2. **Phương diện Động thái Thuật toán & Hiệu năng Trực quan hóa:**  
   Khảo sát mối quan hệ giữa ngưỡng hỗ trợ tối thiểu với không gian ứng viên sinh ra, khối lượng mẫu phát hiện và thời gian thực thi của thuật toán Apriori [2] trên tập dữ liệu thực nghiệm. Đồng thời, đo lường hiệu quả cắt tỉa thực tế của tính chất Apriori qua từng bậc $k$, và đánh giá hiệu năng hiển thị của các công nghệ thư viện đồ họa web (DOM/SVG vs. HTML5 Canvas) [5], [6] khi xử lý các khối lượng dữ liệu điểm phân tán khác nhau.
...
- **Mục tiêu 2:** Hiện thực hóa động cơ khai phá Apriori và sinh luật kết hợp độc lập, mô-đun hóa, tích hợp bộ thu thập số liệu cắt tỉa (*pruning instrumentation*) chi tiết theo từng cấp độ [2].
- **Mục tiêu 3:** Thiết lập khung thực nghiệm chuẩn hóa, bảo đảm tính tái lập (*reproducibility*) thông qua quản lý dữ liệu và mã băm SHA-256.

### 3.2 Các Câu Hỏi Nghiên Cứu (Research Questions)
Ba câu hỏi nghiên cứu hình thức được thiết lập:
...
Độ hỗ trợ của luật $X \Rightarrow Y$ là độ hỗ trợ của tập hợp $X \cup Y$ [1]:
...
### 4.4 Độ Nâng (Lift)
Độ nâng (*lift*) đo lường mức độ phụ thuộc thống kê giữa $X$ và $Y$, xác định xem sự xuất hiện của $X$ có làm thay đổi khả năng xuất hiện của $Y$ so với trường hợp độc lập [4]:
...
- $\text{lift} = 1$: $X$ và $Y$ độc lập thống kê [4].
...
*Nếu một tập mục $X$ không phổ biến ($\text{supp}(X) < \text{minsup}$), thì mọi tập siêu $Y \supset X$ đều không phổ biến và có thể bị loại bỏ mà không cần đánh giá độ hỗ trợ.*
...
1. **Bước Khởi tạo ($k=1$ — `singleton_scan`):** Quét dữ liệu giao dịch để đếm độ hỗ trợ của từng mục đơn $i \in \mathcal{I}$, trích xuất $L_1$.
...
Trong tài liệu khai phá dữ liệu, thuật toán FP-Growth (*Frequent Pattern Growth*) của Han, Pei, Yin (2000) [3] được giới thiệu như một giải pháp thay thế tránh việc sinh ứng viên tường minh:
- **Nguyên lý:** FP-Growth nén dữ liệu giao dịch vào cấu trúc cây tiền tố liên kết bộ nhớ (*FP-tree*), sau đó sử dụng chiến lược chia để trị (*divide-and-conquer*) để khai phá các mẫu phổ biến từ các cây điều kiện (*conditional pattern bases*) [3].
- **Đánh đổi lý thuyết:** Trong khi FP-Growth tránh việc sinh ứng viên tổ hợp [3], Apriori [2] thể hiện cấu trúc phân tầng $k$-itemset, cho phép quan sát trực tiếp động thái cắt tỉa tổ hợp qua từng bước. Trong khuôn khổ đề tài này, Apriori được lựa chọn triển khai nhằm phục vụ mục tiêu phân tích chẩn đoán cơ chế cắt tỉa; FP-Growth được trình bày như nền tảng đối sánh lý thuyết; hiệu năng so sánh phụ thuộc vào tập dữ liệu và cách hiện thực hóa cụ thể.
...
- **Chart.js & Apache ECharts:** ECharts [6] và Chart.js [9] cung cấp mô hình cấu hình biểu đồ khai báo cấp cao. Trong thực nghiệm RQ3, Chart.js 4.4.8 [9] và Apache ECharts 5.6.0 [10] đều được cấu hình sử dụng công nghệ HTML5 Canvas rasterization.
...
Hệ thống sử dụng kiến trúc phân lớp nhẹ với endpoint PHP và các lớp miền, HTTP và persistence được phân tách trách nhiệm theo quyết định kiến trúc ADR-001:
...
Luồng khai phá trong hệ thống được thực thi **đồng bộ** trong một vòng đời yêu cầu HTTP:
...
Nghiên cứu sử dụng tập dữ liệu chuẩn hóa **UCI Mushroom** (`agaricus-lepiota.data`):
- **Siêu dữ liệu công bố từ nguồn UCI [7]:** Báo cáo gốc từ kho lưu trữ UCI ghi nhận 8,124 bản ghi quan sát phân loại, bao gồm 22 thuộc tính hình thái mô tả sinh học và 1 thuộc tính phân lớp mục tiêu (tương ứng hai nhãn ăn được hoặc có độc), cho phép chứa các giá trị khuyết thiếu. Tập dữ liệu được phân phối dưới giấy phép Creative Commons Attribution 4.0 International (CC BY 4.0), định danh DOI chính thức: `10.24432/C5959T` [7].
- **Mô hình biểu diễn tiếp nhận nội bộ của dự án (Project Ingestion Representation):** Trong quy trình tiếp nhận vị trí cố định (*positional ingestion profile*) được đóng băng của dự án (nguồn: `experiments/configs/dataset_manifest.json`), mỗi dòng dữ liệu vật lý bao gồm 23 trường thuộc tính (gồm trường lớp nhãn $c_1$ và 22 trường thuộc tính hình thái sinh học từ $c_2$ đến $c_{23}$). Bộ phân tích của dự án mã hóa từng mục theo tiền tố vị trí $c_j=\text{value}$, định lượng chính xác **119 mục phân loại phân biệt** trên toàn bộ 8,124 giao dịch. Ký tự khuyết thiếu `'?'` được bảo toàn nguyên vẹn như một giá trị phân loại hợp lệ phân biệt ($c_{12}=?$).
...
Bảng 1 tổng hợp toàn bộ kết quả thực nghiệm hình thức cho RQ1 trên tập dữ liệu UCI Mushroom [7].

#### Bảng 1: Tóm Tắt Ảnh Hưởng Của Ngưỡng Hỗ Trợ Đến Không Gian Tìm Kiếm, Khối Lượng Mẫu và Thời Gian Thực Thi (RQ1)
...
```text
[Hình 1: Thời Gian Thực Thi Apriori theo Ngưỡng Hỗ Trợ Tối Thiểu (RQ1)]
File: experiments/figures/F1_apriori_runtime_vs_support.svg
Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả.
```

2. **Không Gian Tìm Kiếm Ứng Viên (Hình 2):**  
...
```text
[Hình 2: Khối Lượng Không Gian Ứng Viên theo Ngưỡng Hỗ Trợ Tối Thiểu (RQ1 / RQ2)]
File: experiments/figures/F2_candidate_volume_vs_support.svg
Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả.
```

3. **Khối Lượng Mẫu Phát Hiện (Hình 3):**  
   Số lượng tập mục phổ biến tăng từ $51$ lên $1,189$ tập mục. Tương ứng, số lượng luật kết hợp thỏa mãn $\text{min\_confidence} \ge 0.75$ tăng từ $223$ lên $11,055$ luật, cho thấy tính nhạy cảm của không gian mẫu đối với ngưỡng lọc hỗ trợ.

```text
[Hình 3: Khối Lượng Tập Mục Phổ Biến và Luật Kết Hợp theo Ngưỡng Hỗ Trợ (RQ1)]
File: experiments/figures/F3_pattern_output_vs_support.svg
Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả.
```

**Trả lời RQ1:** Trong phạm vi tập dữ liệu thực nghiệm UCI Mushroom và cấu hình hiện tại, việc hạ ngưỡng hỗ trợ tối thiểu $\text{min\_support}$ mở rộng đáng kể không gian tìm kiếm ứng viên, khối lượng tập mẫu phổ biến / luật kết hợp và thời gian thực thi trung vị của thuật toán Apriori, phù hợp với động thái tổ hợp của không gian tìm kiếm.

---

### 8.2 Kết Quả RQ2: Động Thái và Hiệu Quả Cắt Tỉa Apriori

Bảng 2 và Bảng T2b (Phụ lục B) cung cấp bằng chứng định lượng về hiệu quả của cơ chế cắt tỉa tập con [2].

#### Bảng 2: Tóm Tắt Tỷ Lệ Cắt Tỉa Ứng Viên Tổng Thể theo Ngưỡng Hỗ Trợ (RQ2)
*(Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả)*
...
#### Phân Tích Động Thái Cắt Tỉa Phân Tầng (Hình 4)
...
```text
[Hình 4: Động Thái Phân Tầng Ứng Viên và Tỷ Lệ Cắt Tỉa Qua Toàn Bộ 5 Ngưỡng Hỗ Trợ (RQ2)]
File: experiments/figures/F4_pruning_dynamics_per_level.svg
Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả.
```

**Trả lời RQ2:** Cơ chế cắt tỉa tập con theo tính chất Apriori loại bỏ từ $5.95\%$ đến $29.28\%$ tổng số ứng viên sinh ra khi ngưỡng hỗ trợ giảm từ $0.60$ xuống $0.35$; việc cắt tỉa tập con thực tế bắt đầu loại bỏ ứng viên từ bậc độ dài $k = 3$, giúp thu hẹp hiệu quả không gian tìm kiếm trước bước đánh giá độ hỗ trợ trên tập dữ liệu giao dịch trong bộ nhớ.

---

### 8.3 Kết Quả RQ3: So Sánh Hiệu Năng Trực Quan Hóa Front-End

Bảng 3 tổng hợp các quan sát độ trễ hiển thị và cập nhật dữ liệu của ba thư viện đồ họa web [8], [9], [10].

#### Bảng 3: So Sánh Độ Trễ Khởi Tạo và Cập Nhật Dữ Liệu của D3.js, Chart.js và Apache ECharts (RQ3)
*(Nguồn: Thực nghiệm đối chứng có kiểm soát của nhóm tác giả)*
...
[Hình 6: Độ Trễ Cập Nhật Dữ Liệu Tại Chỗ theo Quy Mô Dữ Liệu (RQ3)]
...
Kết quả thực nghiệm từ RQ1 và RQ2 cung cấp những hiểu biết cho việc thiết kế các hệ thống khai phá dữ liệu web:
- **Tác động của không gian mẫu bùng nổ:** Khi hạ ngưỡng hỗ trợ, số lượng ứng viên và luật kết hợp tăng nhanh. Nếu không có cơ chế giới hạn bảo vệ (*guardrails*) và chính sách trả về dữ liệu Top-N rút gọn phía API, máy chủ web có thể đối mặt với nguy cơ cạn kiệt bộ nhớ hoặc vượt quá giới hạn thời gian phản hồi HTTP.
- **Vai trò bảo vệ của Cắt tỉa Apriori:** Tỷ lệ cắt tỉa tăng dần lên gần $30\%$ cho thấy tính chất Apriori [2] ngăn chặn hàng trăm ứng viên không phổ biến tham gia vào bước đánh giá độ hỗ trợ trong bộ nhớ, góp phần tối ưu hóa quá trình tính toán backend.
...
2. **Hiệu Chỉnh Ma Trận Hỗ Trợ:** Ma trận hỗ trợ chính thức $[0.35 - 0.60]$ được hiệu chỉnh sau bước thăm dò khả thi tiền hình thức để bảo đảm tính khả thi tính toán trên hệ thống; các ngưỡng hỗ trợ rất thấp ($< 0.30$) chưa được đo lường do giới hạn thời gian chạy và giới hạn số lượng luật.
3. **Thiếu Đường Cơ Sở Đo Thời Gian Không Cắt Tỉa:** Do hệ thống không triển khai phiên bản Apriori thô loại bỏ hoàn toàn bước tỉa, hiệu quả cắt tỉa chỉ được định lượng qua số lượng ứng viên và tỷ lệ cắt tỉa, không suy diễn tỷ lệ tăng tốc thời gian thực thi (*wall-clock speedup*).
...
- Xây dựng hệ thống ứng dụng web bảng điều khiển tương tác, tích hợp giữa tầng xử lý nghiệp vụ PHP phân lớp nhẹ [13], cơ sở dữ liệu MySQL [14] và giao diện người dùng ECharts [10] / Bootstrap [11] đáp ứng.
- Động cơ khai phá Apriori được hiện thực hóa với kiến trúc mô-đun hóa [2], cung cấp số liệu chẩn đoán cắt tỉa chi tiết qua từng cấp độ $k$.
- Thiết lập khung thực nghiệm có kiểm soát với quy trình đóng băng dữ liệu nghiêm ngặt, trả lời ba câu hỏi nghiên cứu RQ1, RQ2 và RQ3 với các bằng chứng thực nghiệm có khả năng tái lập.

---

## 12. HƯỚNG PHÁT TRIỂN TƯƠNG LAI (FUTURE WORK)

Trong giai đoạn tiếp theo, dự án định hướng mở rộng các nội dung sau:
...
7. **[7] UCI Machine Learning Repository: Mushroom Data Set.** *Mushroom [Dataset].* (1981). Donor: Jeff Schlimmer (1987). DOI: [10.24432/C5959T](https://doi.org/10.24432/C5959T). URL: [https://archive.ics.uci.edu/dataset/73/mushroom](https://archive.ics.uci.edu/dataset/73/mushroom).
8. **[8] Bostock, M., & D3 Contributors (2024).** *D3.js: JavaScript library for visualizing data (Version 7.9.0).* Authoritative: [https://d3js.org/](https://d3js.org/). Package: `npm:d3@7.9.0`.
9. **[9] Chart.js Open Source Project (2025).** *Chart.js: Simple yet flexible JavaScript charting for designers & developers (Version 4.4.8).* Authoritative: [https://www.chartjs.org/](https://www.chartjs.org/). Package: `npm:chart.js@4.4.8`.
10. **[10] The Apache Software Foundation (2024).** *Apache ECharts: An Open Source JavaScript Visualization Library (Version 5.6.0).* Authoritative: [https://echarts.apache.org/](https://echarts.apache.org/). Package: `npm:echarts@5.6.0`.
11. **[11] Bootstrap Authors (2025).** *Bootstrap: Powerful, extensible, and feature-packed frontend toolkit (Version 5.3.8).* Authoritative: [https://getbootstrap.com/](https://getbootstrap.com/). Package: `npm:bootstrap@5.3.8`.
12. **[12] OpenJS Foundation (2023).** *jQuery: Fast, small, and feature-rich JavaScript library (Version 3.7.1).* Authoritative: [https://jquery.com/](https://jquery.com/).
13. **[13] The PHP Group.** *PHP Manual: Language Reference & Runtime Architecture.* n.d. Measured Runtime: PHP 8.3.30 (Formal Benchmark Environment) / Language Target: PHP 8.2+. URL: [https://www.php.net/docs.php](https://www.php.net/docs.php) (Accessed: 2026-08-21).
14. **[14] Oracle Corporation.** *MySQL 8.4 Reference Manual: InnoDB Storage Engine & Performance.* n.d. Measured Server: MySQL 8.4.3 (Formal Benchmark Environment). URL: [https://dev.mysql.com/doc/refman/8.4/en/](https://dev.mysql.com/doc/refman/8.4/en/) (Accessed: 2026-08-21).
...
## PHỤ LỤC A — TÍNH TÁI LẬP VÀ NGUỒN GỐC BẰNG CHỨNG (REPRODUCIBILITY & EVIDENCE PROVENANCE)

Nhằm bảo đảm tính minh bạch và khả năng tái lập độc lập của toàn bộ các kết quả thực nghiệm được trình bày trong báo cáo:

1. **Bộ Dữ Liệu Thực Nghiệm Gốc:**
   - Tệp nguồn: `experiments/datasets/agaricus-lepiota.data`
   - Mã băm SHA-256: `3b49f2b87ffbfcaec349cfdcc445fcba35e23f03b417855b7ec6f2e8be56cf01`
   - Siêu dữ liệu đóng băng: `experiments/configs/dataset_manifest.json`

2. **Dữ Liệu Thực Nghiệm Thô & Tóm Tắt (Canonical Evidence Artifacts):**
...
## PHỤ LỤC B — BẢNG SỐ LIỆU CẮT TỈA PHÂN TẦNG CHI TIẾT (APPENDIX - TABLE T2b)
