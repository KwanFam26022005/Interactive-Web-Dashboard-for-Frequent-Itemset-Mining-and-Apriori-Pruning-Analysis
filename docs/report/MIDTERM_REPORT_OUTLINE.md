...
- Đóng góp chính của đề tài: Xây dựng hệ thống bảng điều khiển web tương tác hoàn chỉnh (PHP/MySQL/AJAX/ECharts) tích hợp động cơ Apriori tối ưu, cơ chế cắt tỉa kiểm soát và phân tích hiệu năng front-end đối chứng.

## 2. Phát Biểu Bài Toán (Problem Statement)
- Thách thức về không gian tìm kiếm tổ hợp mở rộng nhanh khi hạ ngưỡng hỗ trợ tối thiểu trong thuật toán Apriori.
- Khoảng cách giữa các công cụ khai phá dữ liệu dòng lệnh (CLI) trừu tượng và nhu cầu trực quan hóa web tương tác.
...
- **Mục tiêu dự án:** Hiện thực hóa kiến trúc web phân lớp nhẹ, xây dựng động cơ Apriori với đo lường cắt tỉa chi tiết và cung cấp giao diện trực quan hóa tương tác.
...
- **RQ3:** Ba thư viện trực quan hóa web D3.js (triển khai SVG), Chart.js (triển khai Canvas) và Apache ECharts (triển khai Canvas) so sánh như thế nào về độ trễ khởi tạo (render) và cập nhật dữ liệu tại chỗ (in-place update) dưới các khối lượng dữ liệu điểm phân tán tương đương?
...
- **4.8 Các thư viện trực quan hóa dữ liệu web:** Giới thiệu đặc tính kiến trúc DOM/SVG và HTML5 Canvas rasterization trong bối cảnh thực nghiệm đối chứng.

## 5. Yêu Cầu Hệ Thống và Ngăn Xếp Công Nghệ (System Requirements and Technology Stack)
- Yêu cầu chức năng: Quản lý tập dữ liệu, cấu hình tham số khai phá, hiển thị bảng dữ liệu, biểu đồ ma trận nhiệt tương quan (heat map), biểu đồ phân tán (scatter plot) và biểu đồ thanh phân tầng.
- Yêu cầu phi chức năng: Tính toàn vẹn dữ liệu, tính đáp ứng giao diện tương tác, giới hạn bảo vệ tài nguyên (guardrails), tính tái lập thực nghiệm tất định.
- Ngăn xếp công nghệ:
  - Back-end: PHP 8.2+ (hướng đối tượng, autoloader tự xây dựng, không framework nặng).
  - Cơ sở dữ liệu: MySQL 8.4 (InnoDB, khóa ngoại, chỉ mục composite tối ưu).
  - Front-end: HTML5, CSS3, Bootstrap 5, JavaScript (ES6+), jQuery, AJAX.
  - Trực quan hóa chính của dashboard: Apache ECharts 5.6.0.
  - Môi trường đối chứng RQ3: D3.js 7.9.0 (SVG), Chart.js 4.4.8 (Canvas), Apache ECharts 5.6.0 (Canvas).

## 6. Kiến Trúc và Hiện Thực Hệ Thống (System Architecture and Implementation)
- **6.1 Kiến trúc tổng thể:** Mô hình kiến trúc phân lớp nhẹ (Lightweight Layered Architecture) với endpoint PHP mỏng và các lớp miền, HTTP, persistence tách bạch.
- **6.2 Tiếp nhận và tiền xử lý dữ liệu (Data Ingestion):** Cơ chế phân tích định dạng giỏ hàng (`BasketCsvParser`, `BasketTextParser`) và định dạng phân loại (`MushroomParser`) thông qua `DatasetImportService`.
- **6.3 Tầng lưu trữ bền vững (Persistence Layer):** Lược đồ cơ sở dữ liệu quan hệ chuẩn (`datasets`, `transactions`, `transaction_items`, `experiment_runs`, `experiment_run_levels`) và chiến lược lưu trữ dữ liệu tóm tắt rút gọn (transient vs. persistent).
- **6.4 Động cơ khai phá Apriori (Apriori Engine):** Thiết kế hướng đối tượng tách biệt các dịch vụ `AprioriEngine`, `CandidateJoiner`, `CandidatePruner`, `SupportCounter`, `FrequentFilter`.
- **6.5 Động cơ sinh luật kết hợp và ma trận tương quan:** Lớp `AssociationRuleGenerator` và `HeatmapBuilder`.
- **6.6 Giao diện lập trình ứng dụng (HTTP JSON API):** Các endpoint `GET /api/datasets.php`, `POST /api/datasets.php`, `POST /api/mining.php` với định dạng JSON chuẩn hóa (RFC 8259) và xử lý đồng bộ phía máy chủ.
- **6.7 Bảng điều khiển web tương tác (Interactive Dashboard):** Tổ chức giao diện Bootstrap đáp ứng, tích hợp tương tác bất đồng bộ AJAX phía client và trực quan hóa Apache ECharts.
...
- **7.3 Giao thức thực nghiệm khai phá (RQ1 / RQ2):** Chế độ chạy hình thức (10 lần lặp độc lập sau 2 lần chạy làm nóng), trộn ngẫu nhiên tất định (seed = 42), đo lường thời gian thực thi trong bộ nhớ và số liệu cắt tỉa.
...
- Đánh giá kiến trúc phân lớp nhẹ PHP/MySQL trong bài toán khai phá dữ liệu tương tác.
...
- Bổ sung cơ chế bộ đệm phân tán (caching) cho các tập mẫu phổ biến thường dùng.

## 13. Tài Liệu Tham Khảo (References)
- Danh mục tài liệu học thuật và tài liệu kỹ thuật chuẩn xác (trạng thái chờ thẩm định chi tiết ở Phase 5B).
